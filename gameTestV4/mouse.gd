	...
	if (LastPressedTime > 0):
		...
		if (LastPressedTime == 0):
			if mouseHeld:
				...
				OnHeldClick(get_global_mouse_position())
				holding = true
			else:
				#OnSingleClick(get_local_mouse_position())
				OnSingleClick(get_global_mouse_position())

func global_send():
	send_global_position.emit(get_global_mouse_position())


# TODO - Differiate between right and left.
func _input(event):
	if event is InputEventMouseButton and event.pressed:
		if (event.button_index == MOUSE_BUTTON_WHEEL_UP || event.button_index == MOUSE_BUTTON_WHEEL_DOWN):
			...
			if (LastPressedTime > 0):
				...
				OnDoubleClick(get_global_mouse_position())
				LastPressedTime = 0
			else:
				LastPressedTime = 0.3
				# set this to held?
